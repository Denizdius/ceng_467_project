#!/usr/bin/env python3
"""
Baseline 3: Qwen3-8B Base (Unsloth 4-bit base), QLoRA on DEITA-6k, save LoRA adapters (PEFT).

Activate environment:
    source ~/Documents/ee_563/.venv/bin/activate

Example:
    python scripts/baseline3_sft_qwen3_8b_4bit_qlora.py --output_dir outputs/baseline3_deita
"""
...
# Unsloth must import before TRL/transformers for patches; avoids broken tokenizer placeholders.
...
def repair_special_tokens(model, tokenizer) -> None:
    """
    Unsloth fast inference / template paths can leave eos_token as '<EOS_TOKEN>' (not in vocab).
    TRL 0.24+ validates eos_token against vocab; sync from model.config.eos_token_id.
    """
...
def parse_args() -> argparse.Namespace:
    p = argparse.ArgumentParser(description="SFT Qwen3-8B Base 4-bit (QLoRA) on DEITA-6k, save LoRA adapters.")
    p.add_argument("--output_dir", type=str, default="outputs/baseline3_deita", help="Root directory for checkpoints.")
...
def build_deita_dataset(tokenizer, max_samples: int | None):
    ...
    def conversations_to_text(examples):
        ...
        texts = []
        for convo in convos:
            # convo is typically a list[dict], e.g. [{"from":"human","value":"..."}, ...]
            messages = []
            ...
            try:
                ...
                # Fallback: simple role-tag formatting.
                parts = []
                for m in messages:
...
def run_pipeline(args: argparse.Namespace, max_seq_length: int) -> None:
    ...
    print(f"[config] model={MODEL_NAME} load_in_4bit=True max_seq_length={max_seq_length}")
    ...
    train_kwargs = dict(
        ...
        # Do not pass template placeholders; TRL validates these strings against vocab.
        eos_token=None,
        pad_token=None,
    )
...
def main() -> None:
    ...
    # Auto-tune for higher VRAM usage: try larger batch first, then fall back.
